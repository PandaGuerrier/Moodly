...
>(({ variant = "solid", setData, errors, name, displayName }, ref) => {
  ...
  return (
    <View
      style={tw`flex items-center justify-center rounded-full px-4 w-full text-md mt-5 `}
    >
      <Text style={tw`text-base text-start w-full px-3`}>{displayName}</Text>
      <TextInput
        onPress={() => setIsClicked(true)}
        onPressOut={() => setIsClicked(false)}
        style={tw`flex items-center justify-center rounded-full py-3 px-2 w-full text-md  ${variantTxt} ${errors[name] ? `border border-red-400 bg-red-200` : ``}`}
        onChangeText={(text) => setData(name, text)}
      >
        <Text></Text>
      </TextInput>
      {errors[name] && (
        <Text style={tw`text-red-600 text-xs mt-1`}>{errors[name]}</Text>
      )}
    </View>
  );
});
